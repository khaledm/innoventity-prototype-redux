...
public class SubmitSalesMarketingResponseTests : IDisposable
{
    ...
    private static Dictionary<string, string[]> InvokeValidateProjection(
        List<Innoventity.API.Features.Bids.SubmitSalesMarketingResponse.YearlySaleRequest>? entries)
    {
        var method = typeof(Innoventity.API.Features.Bids.SubmitSalesMarketingResponse)
    ...
    [Fact]
    public async Task SubmitSalesMarketingResponse_BlankParticipationType_Returns400()
    {
        var token = await GetAccessToken("sales@submitsales.test", "SalesMarketing");
        var request = new
        {
    ...
    [Fact]
    public async Task SubmitSalesMarketingResponse_OverlongParticipationType_Returns400()
    {
        var token = await GetAccessToken("sales@submitsales.test", "SalesMarketing");
        var request = new
        {
            location = "Americas",
            participationType = new string('S', 101),
            participationProposal = MakeProposal(),
            yearlySales = MakeValidYears()
        };
    ...
    public void ValidateProjection_NegativeNumericValues_ReturnsValidationErrors()
    {
        ...
        Assert.Contains("YearlySales[0].UnitsSold", errors.Keys);
