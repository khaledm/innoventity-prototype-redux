...
variable "swa_hostname" {
  type        = string
  description = "Default hostname of the Azure Static Web App (without https://) — wired from module.static_web_app.default_host_name. Added to CORS allowed_origins so the SPA can call the API cross-origin."
}
...
resource "azurerm_linux_web_app" "main" {
  ...
  site_config {
    ...
    cors {
      # Always include the actual SWA hostname (auto-generated by Azure, not predictable).
      # For dev also allow localhost:4200 so local ng serve can call the deployed API.
      allowed_origins = var.environment == "dev" ? [
        "http://localhost:4200",
        "https://${var.swa_hostname}",
        ] : [
        "https://${var.swa_hostname}",
      ]
