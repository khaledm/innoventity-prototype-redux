...
public static class SubmitManufacturingResponse
{
    ...
    internal static Dictionary<string, string[]> ValidateProjection(List<YearlyManufacturingCostRequest>? entries)
    {
        ...
        {
            errors["YearlyManufacturingCosts"] = [$"Projection years must be contiguous starting from year 1 — invalid year(s): {string.Join(", ", invalidYears)}."];
        }
        else if (duplicateYears.Count > 0)
        {
            errors["YearlyManufacturingCosts"] = [$"Projection years must be contiguous starting from year 1, with no duplicates — duplicate year(s): {string.Join(", ", duplicateYears)}."];
        }
        else
        {
            ...
            if (!distinctYears.SequenceEqual(expectedYears))
            {
                ...
                errors["YearlyManufacturingCosts"] = missingYears.Count > 0
