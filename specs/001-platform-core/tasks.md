...
- [ ] T076 Author three GitHub Actions workflows: (1) `.github/workflows/infra.yml` with jobs `terraform-plan-data` → `approve-data` (manual gate, prod only) → `terraform-apply-data` → `terraform-plan-core` → `terraform-apply-core` → `pester-infra` — **AUTHORED** commit `pending` — `terraform-apply-core` injects `-var sql_server_id=...` from data/ outputs (C5, FR7.6 ✅); (2) `.github/workflows/deploy.yml` with jobs `preflight` → `build-test` → `migrate` → `deploy` → `pester-health` → `slot-swap` — **AUTHORED**; (3) `.github/workflows/drift.yml` with `cron: "0 2 * * *"` trigger only, jobs `drift-check-core` + `drift-check-data` using `terraform plan -detailed-exitcode` (detection only, never applies) — **AUTHORED**; **Gate**: before merging T076 PR, run `/speckit.analyze` and confirm zero new CRITICAL findings
