...
- [ ] T048 Run `dotnet stryker` from `src/Innoventity.API/` — verify mutation score ≥ 80% across new feature files (SC-005) — **FAILED**: overall 59.48% (273 killed / 459 testable), below the `break: 71` threshold. New handlers are weak: SubmitInvestorResponse.cs 44.8%, SubmitManufacturingResponse.cs 48.2%, SubmitSalesMarketingResponse.cs 50.6%, SubmitResearchDevelopmentResponse.cs 51.7%, GetBids.cs 63.0%. SelectPartners.cs (89.7%, pre-existing) unaffected. Survivors are mostly unasserted error-message text and unasserted boundary values (e.g. proposal length exactly 100, year exactly 10, rationale exactly 20/500 chars) — tests check status codes but not exact messages/boundaries. Needs test hardening before this gate passes.
